...
Карточка `332×178 pt`, падинг `--card-pad` 12 → ички қути `308×154 pt`, радиус `--r-card` 18, фон `--c-card` `#FFFFFF`, соя `--shadow-card` `0 6 18 rgba(28,39,76,.06)`. Тик оқим, элементлар ораси `--s-2` 8 pt.
...
|---|---|---|---|---|---|
| Сарлавҳа қатори | `.wcard__head` | 308×35 (сатр билан) ёки 308×18 | — | — | `align-items:flex-start`; чип фақат `extra-missed` да |
...
| Тана | `.wcard__body` | 308×67 / 84 (flex 1) | — | — | Устун, оралиқ 6 |
| Таймер | `.wtimer` | 308×45; 4 устун × 72,5, gap 6 | — | — | `data-cd="KK:SS:DD:SS"`, ҳар сонияда 4 та `<b>` янгиланади |
| Таймер блоки | `.wtimer__b` | 72,5×45, падинг `6 0 5`, радиус `--r-inner` 12 | рақам 600 20/23, ls −.5, tabular-nums; ёрлиқ 500 9/11, ls .3, UPPERCASE | фон `--c-row` `#F8F8FC`; рақам `--c-text` `#000000`; ёрлиқ `--c-text-3` `#B4B4B4` | Ёрлиқлар: `KUN` `SOAT` `DAQIQA` `SONIYA` |
...
| Изоҳ | `.wcard__note--pink` | ≤ 2 қатор → 46 pt (2×15 + 8 + 8), падинг `8 10`, радиус 12 | 500 12/15 | фон `--c-pink-soft` `#FFE4EA`, матн `--c-text` | Фақат `extra-missed`; робот бор → `.ex-pad`, кенглик 236 pt, ≤ 54 белги |
| Кулранг матн | `.wcard__text.wcard__muted` | 1 қатор 16 pt | `--t-body` 500 13/16 | `--c-text-2` `#999999` | Фақат `empty`, `.ex-pad--80` → кенглик 228 pt |
...
| Асосий тугма | `.wcard__btn` | 36 pt, радиус `--r-btn` 12, падинг `0 14`, `flex:1` | `--t-btn` 600 13/16 | фон `--c-brand` `#FF4F28`, матн `#FFFFFF` | ibtn билан бирга кенглиги 268 pt; ёлғиз бўлса 308 pt; `extra-missed` да 236 pt |
...
Кенглик текшируви (SF Pro тахминий: 15/600 ≈ 8,3 pt/белги, 12/600 ≈ 6,6): `Qo'shimcha dars` (124) + 8 + `Kelmadi` чипи (62) = 194 ≤ 308. Таймер устуни 72,5 pt — энг узун ёрлиқ `SONIYA` ≈ 36 pt, рус `СЕКУНДЫ` ≈ 43 pt, иккиси ҳам сиғади.
...
|---|---|---|---|---|
| `extra-upcoming` **(асосий)** | `status ≠ no_show`, дарс куни бугун эмас, `canCancel` | Чип йўқ; `.wtimer` нейтрал, `02:03:15:00` (2 кун 3 соат 15 дақиқа) | `Batafsil` (268 pt) + ўчириш ibtn `close.svg` | `extra` / ibtn → `cancel-confirm` |
...
| `extra-missed` | `status = no_show` | Пушти чип `Kelmadi`; **таймер йўқ**; қизил сатр, пушти изоҳ, `time_up.png` 56 pt | Ягона тугма `Mentorga yozish` (236 pt) | `extra-missed` |
...
|---|---|---|---|
| Сарлавҳа | `Qo'shimcha dars` | `Доп. занятие` | Тўлиқ шакл `Дополнительное занятие` ≈183 pt: таймерли ҳолатларда сиғади, лекин `extra-missed` да чип билан 308 pt га тақалади — қисқа шакл қолдирилсин |
| Сатр | `15 sentabr, 16:00` / `Bugun, 16:00` | `15 сентября, 16:00` / `Сегодня, 16:00` | Тошкент куни |
| Чип | `Kelmadi` | `Не пришёл` | Илова сатри |
| Таймер ёрлиқлари | `KUN` `SOAT` `DAQIQA` `SONIYA` | `ДНИ` `ЧАСЫ` `МИНУТЫ` `СЕКУНДЫ` | Устун 72,5 pt — ҳаммаси сиғади |
...
- **Карточка** — `SizedBox(332, 178)` → `DecoratedBox` (радиус 18, соя `0 6 18 rgba(28,39,76,.06)`) → `ClipRRect` → `Stack`: 1) `Padding(12)` + `Column(crossAxisAlignment.stretch)`: head `Row(crossAxisAlignment.start)`, `SizedBox(8)`, `Expanded(body)`, `SizedBox(8)`, foot `Row(spacing 8)`; 2) робот `Positioned(right: 8, bottom/top: 8)`. Робот бўлса тана ва пастки қаторга `Padding(right: 72)` (`empty` да 80).
