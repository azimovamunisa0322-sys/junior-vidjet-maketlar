...
|---|---|---|---|---|
| `.wcard__head` | **21 pt**; сатр остида сана бўлса **39 pt** (21 + 2 + 16) | — | — | чапда матн блоки, ўнгда чип |
| `.wcard__title` | 1 қатор | **17/600, lh 21**, ls −0.1 (`--t-title`) | `--c-text #000000` | доим `To'lov` |
| `.wcard__sub` | 1 қатор, ≤ **27** белги | **13/500, lh 16** (`--t-small`) | default `--c-text-2 #999999`; `--brand #FF4F28` (1–3 кун, бугун); `--red #ED0000` (ўтган, музлатилган); `--blue #1CB0F6` (демо) | сана / ҳолат |
| `.wcard__chip` | баландлик 22, padding 0 8, pill | **13/600** | default фон `--c-row #F8F8FC` матн `#999999`; `--brand` фон `--c-brand-soft #FFEDE7` матн `#FF4F28`; `--pink` фон `--c-pink-soft #FFE4EA` матн `#ED0000`; `--yellow` фон `#FCF8DC`, чегара 1 pt `#E9BB36`, матн `#D9A621` | иконка 16 pt (`.ic` mask) ёки танга расми 14 pt; иконка–матн оралиғи 4 |
| `.wcard__body` | **77 pt** (flex: 168 − 39 − 8 − 8 − 36); хато ҳолатда **95 pt** (сатрсиз сарлавҳа 21). Мазмуни: сумма қолипида **50**, изоҳ қолипида **48** | — | — | флекс, `gap` 6 |
| `.wcard__num` (сумма) | 1 қатор **26 pt**, ≤ **13** белги | **22/600, lh 26** (`--t-num`), ls −0.3 | `#000000` | `450 000 so'm` (12 белги) — мингликлар бузилмас бўш жой билан; ≈ **135 pt** (аввал ≈ 123) |
| `.wcard__text.wcard__muted` (изоҳ) | 1 қатор **18 pt** | **14/500, lh 18** (`--t-body`) | `#999999` | ≤ **26** белги (робот бор бўлса ≤ 248 pt кенглик) |
| `.wcard__note` | 2 қатор, padding 8 10, радиус 12 → **48 pt** (2×16 + 16) | **13/500, lh 16** | `--pink` фон `#FFE4EA`; `--blue` фон `#E8F6FE`; матн `#000000` | ≤ **70** белги (аввал 76); `overdue`, `frozen`, `demo-only` да сумма ўрнига |
...
| `.wcard__btn` | баландлик 36, padding 0 14, радиус 12 | **14/600, lh 17** (`--t-btn`) | оқ матн, фон `#FF4F28`; `--ghost` фон `#F8F8FC` матн `#000000` | робот бор ҳолатда `--fit` (мазмун кенглигида, ≈ **105 pt**, аввал 98), қолганларда тўлиқ кенглик |
| `.wcard__ibtn` + `.pay-i` | 32×32, радиус 10; ичида 16 pt доира «i», ҳалқа 1.5 pt | **11/600 lh 16** — `cards/payment.html` да **қатъий** ёзилган, `--t-tiny` эмас → шрифт шкаласи ўзгарганда ҳам **ўзгармади** | фон `#F8F8FC`, ҳалқа ва «i» `--c-slate #93A2C0` | иловада info иконкаси йўқ — матнли «i» |
| `.wcard__art` | 64×64 (роботлар) / 56×56 (`ta_time`) | — | — | `position: absolute; right: 8; bottom: 8`, `pointer-events: none` |

Баландлик йиғиндиси (ички қути 168 pt). Шрифт шкаласи катталашгач сарлавҳа қатори **35 → 39**, сатрсиз сарлавҳа **18 → 21** бўлди; карточка баландлиги ўзгармагани учун фарқ `flex` танадан (`.wcard__body`) олинди: тана **81 → 77** (хато ҳолатида **98 → 95**). Тана мазмуни ҳам ўсди: сумма қолипи 46 → **50** (сумма 26 + `gap` 6 + изоҳ 18), изоҳ қолипи 46 → **48**.

| Ҳолат | Сарлавҳа | Тана (flex) | Тана мазмуни | Бўш жой | Йиғинди |
|---|---|---|---|---|---|
| `days-left`, `due-soon`, `due-today`, `bonus` | 39 | 77 | 50 | 27 | 39 + 8 + 50 + 8 + 36 = **141** |
| `overdue`, `frozen`, `demo-only` | 39 | 77 | 48 (изоҳ) | 29 | 39 + 8 + 48 + 8 + 36 = **139** |
| `loading` | 33 (скелет, қатъий) | 83 | 42 | 41 | 33 + 8 + 42 + 8 + 36 = **127** |
| `error` | 21 | 95 | 42 (18 + 6 + 18) | 53 | 21 + 8 + 42 + 8 + 36 = **115** |

Ҳамма ҳолатда заҳира 27 pt дан кам эмас — шрифт катталашганидан кейин ҳам тошиш хавфи йўқ. Эслатма: скелет `.pay-sk-num` ҳамон **24 pt**, ҳақиқий сумма қатори эса энди **26** — иловада 26 қилинсин (2 pt, сезилмайди).

Кенглик — **қайта ҳисобланди** (эски ўлчовлар янги шрифт нисбатига кўпайтирилди: sub ва чип ×13/12, body ×14/13, title ×17/15; рақамлар **тахмин**, рендерда текширилсин). 320 pt қаторда энг кенг сарлавҳа блоки + чип — `frozen`: 130 + 8 + 108 = **246** (аввал 228); `overdue`: 103 + 8 + 125 = **236** (аввал 218); RU да `Просрочено на 3 дня` чипи билан **≈ 281** (аввал 259) — 320 pt га сиғади, лекин захира 61 дан **39 pt** га тушди, RU чип матни бундан узун бўлмасин. Робот остидаги матн ҳамон 248 pt дан кам: `To'lov muddati bugun tugaydi` **179** (аввал 166), `Internetni tekshirib, qayta urining` **197** (аввал 183), RU `Срок оплаты истекает сегодня` **197** — учаласи ҳам сиғади, захира ≈ 51 pt.
...
- `amount ≤ 0` ёки йўқ → сумма қатори ўрнида битта қатор `Keyingi to'lov` (**14/500** `#999999`); панелда ҳам сумма қатори яширилади.
