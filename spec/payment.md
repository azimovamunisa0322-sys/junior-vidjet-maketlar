...
Карточка: 304×178 pt, padding 12 → ички қути 280×154 pt, радиус 18 (`--r-card`), фон `--c-card #FFFFFF`, соя `--shadow-card`. Ички элементлар вертикал, оралиқ 8 pt (`gap`). Матн семантикаси: апельсин = бренд/яқин муддат, пушти-қизил = ўтган/музлатилган, сариқ = танга бонуси, кўк = маълумот (демо), slate = ноактив.
...
| `.wcard__text.wcard__muted` (изоҳ) | 1 қатор | 13/500, lh 16 | `#999999` | ≤ 28 белги (робот бор бўлса ≤ 208 pt кенглик) |
...
Кенглик (SF Pro метрикасидан ўлчанган, 280 pt қаторда): энг кенг сарлавҳа блоки + чип — `frozen`: 120 + 8 + 100 = 228; `overdue`: 95 + 8 + 115 = 218; RU да `Просрочено на 3 дня` чипи билан 259. Робот остидаги матн 208 pt дан кам: `To'lov muddati bugun tugaydi` 166, `Internetni tekshirib, qayta urining` 183, RU `Срок оплаты истекает сегодня` 183.
...
|---|---|---|---|
| Карточка сирти (чип, сана, сумма, изоҳ) | 304×178 | tap | Тўлов пастки панели (§ 5). `loading` да ҳеч нарса; `error` да «Qayta urinish» билан бир хил |
...
- **Тузилиш:** `SizedBox(304×178)` → `DecoratedBox` (радиус 18, соя `0 6 18 rgba(28,39,76,.06)`) → `Padding(12)` → `Stack` [ `Column` (head `Row`, `Expanded(body)`, foot `Row`), `Positioned(right: 8, bottom: 8, child: IgnorePointer(Image.asset))` ]. Робот бор ҳолатда body матнига `Padding(right: 72)`.
