...
Карточка `297×171 pt`, падинг `12`, ички қути `273×147`, радиус `--r-card` 18, фон `--c-card` `#FFFFFF`, соя `--shadow-card`. Тик оралиқ блоклар орасида `--s-2` 8 pt.
...
| `.rc-name` курс номи (`.wcard__num`) | 1 қатор, устун кенглиги 193 pt, `…` | 20/600, line-height 24, letter-spacing −0.3 | `--c-text` | ≈ 16–18 лотин белги сиғади; тўлиқ ном варақда |
| `.rc-cap` ҳолат сатри (`.wcard__muted`) | 1 қатор, `…` | 12/500, line-height 15 | `--c-text-2` `#999999` | `Sizga mos kurs` / `Davom etmoqda` |
| `.wcard__progress` | баландлик 6, радиус 3, тўлиқ кенглик 273 | — | трек `--c-line-2` `#F0F0F0`, тўлдириш `--c-brand` | фақат `in-progress`/`no-image`; кенглик = фоиз |
...
Баландлик ҳисоби (энг тўлиқ ҳолат `in-progress`): head 22 + 8 + body 52 + 8 + foot 36 = **126 ≤ 147**. `not-started`: 22 + 8 + 40 + 8 + 36 = 114. `error`: 18 + 8 + (16 + 6 + 15) + 8 + 36 = 107. Қолган жой `.wcard__body` га (`flex: 1`) тушади, футер пастга ёпишади.

Матн узунлиги (SF Pro файллари билан CoreText'да ўлчанган): сарлавҳа 15/600 = 132 pt (RU «Рекомендуемый курс» 156); чип «Boshlanmagan» = 98 pt («Не начат» 68) — иккаласи бирга 273 га сиғади. Курс номи 20/600 устунда 193 pt: «Web dasturlash» 138, «Matematika» 105, «Grafik dizayn» 116, «Blockly Dasturlash» 164, «Веб-разработка» 153 сиғади; «Графический дизайн» (198) ва 30–50 белгили реал номлар `…` билан кесилади («Grafik dizayn va animatsiya asoslari» → «Grafik dizayn va ani…») — бу кутилган хатти-ҳаракат, тўлиқ ном варақда.
...
Алоҳида чизилмаган, лекин қоидаси бор вариантлар:
...
|---|---|---|---|
| `.wcard__btn` `Boshlash` / `Davom etish` | 36 pt баландлик, кенглиги 273 − 40; Flutter'да `minHeight: 44` ёки ота-контейнер падинги билан 44 | `Number(course.id)` truthy → курс кириш маршрути (веб `getCourseEntryPath` муқобили); акс ҳолда каталог | Илова ичидаги маршрут (`push`): курс саҳифаси (roadmap / кириш экрани) ёки `Kurslar` каталоги. Эгалик текширилмайди — веб паритети; paywall кераклиги очиқ савол |
...
Ҳали чизилмаган; мазмун карточкадан кўчирилган элементлардан иборат. Илова услубида: тортиш дастаси, радиус 20, ёпиш ≥ 44 pt, Android back / свайп ёпади.

1. Курс баннери катта (илова курс карточкаси нисбатида, ≈ 216×123, тўлиқ кенглик) ёки заҳира (`--c-brand-soft` + `book.svg`).
2. Ёрлиқ: бэкенд `title` ёки `Tavsiya etilgan kurs` (12/500 `--c-text-2`).
3. Тўлиқ курс номи, 2–3 қатор, 22/600.
4. Ҳолат қатори: чип (`Boshlanmagan` / `45%`) + прогресс чизиғи (бошланганда).
5. Изоҳ: бэкенд `message`; йўқса стандарт матн — бошланмаган: «Bu kurs sizning yo'nalishingizga mos. Birinchi darsdan boshlang.»; бошланган: «Kursni davom ettiring va natijangizni ko'taring.» (иловадаги «Shunday davom eting va natijangizni ko'taring!» оҳангида). Пушти эмас — `--c-row` ёки `--c-blue-soft` (маълумот).
6. Агар API берса: модуллар/дарслар сони («12 ta dars»), давомийлик, нарх — ҳозирча веб маълумотида йўқ (очиқ савол).
7. Ёпишиб турувчи асосий тугма ≥ 48 pt: `Boshlash` / `Davom etish` (карточка тугмаси билан бир хил маршрут).
8. Иккиламчи ҳавола: `Mos kurslarni ko'rish` → каталог (`Kurslar`).
