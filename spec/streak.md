...
Карточка `297×171 pt`, padding `12`, ички қути `273×147`, радиус `--r-card` 18, фон `--c-card` `#FFFFFF`, соя `--shadow-card`. Блоклар орасидаги вертикал оралиқ `--s-2` 8 pt. Тана (`.wcard__body`) қолган жойни олади, мазмуни вертикал марказда.
...
| `.wk` — ҳафта қатори | 273×44 | — | — | 7 устун × 39 pt, `space-between` |
...
| `.wcard__note--brand` (`new-user`) | 46 pt (2 қатор), padding 8 10, радиус 12, тананинг юқорисига ёпишган | 12/500 | фон `--c-brand-soft`, матн `--c-text` | ≤ 2 қатор, ≈ 70 белги; робот бошидан ≥ 4 pt юқорида тугайди |
...
Баландлик ҳисоби (ички 147 pt): `normal / record / today-pending / frozen-today / lost / loading` = 35 (сарлавҳа + сатр) + 8 + 44 (қатор) + 8 + 36 = **131**; `new-user` = 35 + 8 + 46 (изоҳ) + 8 + 36 = **133**; `error` = 18 + 8 + 32 + 8 + 36 = **102**. Ҳеч бир ҳолатда скролл ва кесилиш йўқ.
...
|---|---|---|---|
| `Kalendar` (`normal`, `record`, `frozen-today`) | ≈ 213×36 | tap | Стрик календари — **тўлиқ экранли варақ** (§5 A) |
| `Olovni oling` (`today-pending`) | ≈ 213×36 | tap | Route: талабанинг жорий дарси (илова «Keyingi darsingiz» мантиғи); дарс йўқ бўлса «Mening kurslarim». Қайтганда `streaks` қайта сўралади |
| `Qaytadan boshlash` (`lost`) | ≈ 213×36 | tap | Ўша route — жорий дарс |
| `Boshlash` (`new-user`) | ≈ 201×36 | tap | Ўша route — биринчи дарс |
...
| Ҳафта қатори `.wk` | 273×44 (зона: тана бўйи 60) | tap | Стрик календари (§5 A) — қатор веб'дагидек «ўлик» эмас |
...
| `Qayta urinish` (`error`) | 273×36 | tap | `streaks` сўровини қайтаради, карточка `loading` га ўтади |
...
- **Виджет дарахти:** `Container` (297×171, `BoxDecoration` радиус 18, соя) → `Padding(12)` → `Column`: `Row` (сарлавҳа блоки `Column` + `Spacer` + чип / `IconButton`), `Expanded(Center(week row))`, `Row` (тугмалар). Чип — `Container` pill + `Row(Image 14, Text)`. Робот — `Stack` + `Positioned(right: 8, bottom: 8)`, тугмалар қаторига `padding.only(right: 72)`.
- **Ҳафта қатори:** `Row` ичида 7 × `SizedBox(width: 39)`; боғловчилар алоҳида `CustomPaint` қатлами доиралар **остида** (`Stack`: паст — чизиқлар, юқори — доиралар), чизиқ `y = 13`, кенглик 2, ранг `brand`. Доира `Container(28, shape: circle, border: 2 шаффоф/brand/blue)`; расм `Image.asset` 18/16.
