...
Карточка `344×192 pt`, padding `12`, ички қути `320×168`, радиус `--r-card` 18, фон `--c-card` `#FFFFFF`, соя `--shadow-card`. Блоклар орасидаги вертикал оралиқ `--s-2` 8 pt. Тана (`.wcard__body`) қолган жойни олади, мазмуни вертикал марказда.

| Элемент | Ўлчам | Шрифт | Ранг | Изоҳ |
|---|---|---|---|---|
| `.wcard__title` — `Streak` | **21 pt** баландлик | **17/21** 600 (`--t-title`) | `--c-text` `#000000` | Ҳамма ҳолатда бир хил |
| `.wcard__sub` — ҳолат сатри | **16 pt**, юқоридан 2 pt | **13/16** 500 (`--t-small`) | одатда `--c-text-2` `#999999`; `--brand` `#FF4F28` (хавф), `--blue` `#1CB0F6` (freeze), `--green` `#58CC02` (рекорд), `--red` `#ED0000` (нолланди) | ≤ 30 белги, бир қатор |
| `.wcard__chip` — сон / вақт | баландлик 22, padding 0 8, pill | **13/16** 600 | `--brand`: фон `--c-brand-soft` `#FFEDE7`, матн `--c-brand`; `--green`: `--c-green-soft` `#ECFFDE` / `--c-green-2`; `--pink`: `--c-pink-soft` `#FFE4EA` / `--c-red` | Ичида расм 14 pt (`small_fire.png`) ёки иконка 14 pt (`time.svg`, қизил), оралиқ 4 |
| `.wcard__ibtn` — «i» | 32×32, радиус 10 | «i» italic 600 **14/1** — `.itxt` да **қатъий** ёзилган, токен эмас → **ўзгармади** | фон `--c-row` `#F8F8FC`, ҳарф `--c-slate` `#93A2C0` | Чип ўрнида, `lost` ва `new-user` да; босиш зонаси 44×44 |
| `.wk` — ҳафта қатори | 320×**45** (28 доира + 2 оралиқ + **15** ёрлиқ) | — | — | ҳар устун қаторнинг 1/7 қисми (320 / 7 = 45,7 pt) — оралиқ қолмайди, устунлар қўшнисига тегиб туради; карточка кенглиги ўзгарса ҳам тақсимот бузилмайди. ⚠️ CSS да `.wk { height: 44px }` **қатъий** — ёрлиқ 14 дан 15 pt га ўсгач мазмун 1 pt чиқади; иловада баландлик **45** қилинсин |
...
| `.d__l` — ҳафта куни | **15 pt**, доирадан 2 pt пастда | **12/15** 500 (`--t-tiny`) | `--c-text-2`; бугун — `--c-brand` 600 (муз бўлса `--c-blue`) | Илованинг ўз 2-ҳарфли жадвали: `Du Se Ch Pa Ju Sh Ya` |
| Боғловчи | устун кенглигича `100%` × 2 pt (≈ 45,7), y = 13 (`left: −50%`) | — | `--c-brand` | Олдинги доира марказидан шу доира марказигача, доиралар **орқасида** (учлари кўринмайди, кўринадиган қисми — доиралар ораси: 45,7 − 28 ≈ 18 pt). Ўлчам фоизда бўлгани учун кенглик ўзгарса ҳам бузилмайди. Кетма-кет фаол/музлатилган кунлар орасида; музлатилган кун узмайди |
| `.wcard__note--brand` (`new-user`) | **48 pt** (2 қатор: 2×16 + 16), padding 8 10, радиус 12, тананинг юқорисига ёпишган | **13/16** 500 | фон `--c-brand-soft`, матн `--c-text` | ≤ 2 қатор, ≈ **64** белги (аввал 70); изоҳ y = 59…107 да тугайди, робот боши (`robot2` 64 pt, y 120 дан) — ораси **13 pt** (аввал ≥ 11 pt деб ёзилган эди) |
| `.wcard__btn` — асосий тугма | баландлик 36, радиус 12, padding 0 14, қолган кенглик | **14/17** 600 (`--t-btn`) | фон `--c-brand`, матн `#FFFFFF`; `--ghost` (`error`): фон `--c-row`, матн `--c-text` | Матн ≤ **16** белги (аввал 18), бир қатор |
| `.frz` — freeze баланси (`--ghost --fit`) | 36 pt баландлик, кенглик ≈ **54** (аввал 52), padding 0 12 | **14/17** 600 | фон `--c-row`, матн `--c-text`; баланс 0 — матн `--c-text-3` `#B4B4B4` | Расм 16 pt `streak_freeze.png` (0 да `inactive_freeze.png`) + сон, оралиқ 4; асосий тугмадан 8 pt ўнгда |
| `.err__ic` (`error`) | 32×32 доира | — | фон `--c-pink-soft`, `x.svg` 16 pt `--c-red` | Матн блоки: **14/500** «Yuklab bo'lmadi» + **13/500** `--c-text-2` «Internetni tekshiring» = **34 pt** — энди иконкадан (32) баланд, қатор баландлигини матн белгилайди |
...
Баландлик ҳисоби (ички 168 pt). Шрифт шкаласи катталашгач сарлавҳа қатори **35 → 39** (21 + 2 + 16), сатрсиз сарлавҳа **18 → 21** бўлди; карточка баландлиги ўзгармагани учун фарқ `flex` танадан олинди: тана **81 → 77** (`error` да **98 → 95**).

- `normal / record / today-pending / frozen-today / lost / loading` = **39** + 8 + **45** (қатор) + 8 + 36 = **136**, қолган **32 pt** — тананинг (`.wcard__body`, `flex: 1`) бўш жойи, қатор шунинг ичида вертикал марказда;
- `new-user` = **39** + 8 + **48** (изоҳ) + 8 + 36 = **139**, қолган **29 pt** изоҳнинг остида (тана `--top`);
- `error` = **21** + 8 + **34** + 8 + 36 = **107**, қолган **61 pt** тананинг бўш жойи.

Ҳеч бир ҳолатда скролл ва кесилиш йўқ — энг тор ҳолатда ҳам 29 pt захира бор.
...
|---|---|---|---|
| `Kalendar` (`normal`, `record`, `frozen-today`) | ≈ 248×36 | tap | Стрик календари — **тўлиқ экранли варақ** (§5 A) |
| `Olovni oling` (`today-pending`) | ≈ 248×36 | tap | Route: талабанинг жорий дарси (илова «Keyingi darsingiz» мантиғи); дарс йўқ бўлса «Mening kurslarim». Қайтганда `streaks` қайта сўралади |
| `Qaytadan boshlash` (`lost`) | ≈ 248×36 | tap | Ўша route — жорий дарс |
| `Boshlash` (`new-user`) | ≈ 248×36 | tap | Ўша route — биринчи дарс |
...
| Ҳафта қатори `.wk` | 320×44 (зона: тана бўйи 81) | tap | Стрик календари (§5 A) — қатор веб'дагидек «ўлик» эмас |
...
| `Qayta urinish` (`error`) | 320×36 | tap | `streaks` сўровини қайтаради, карточка `loading` га ўтади |
...
- **Виджет дарахти:** `Container` (344×192, `BoxDecoration` радиус 18, соя) → `Padding(12)` → `Column`: `Row` (сарлавҳа блоки `Column` + `Spacer` + чип / `IconButton`), `Expanded(Center(week row))`, `Row` (тугмалар). Чип — `Container` pill + `Row(Image 14, Text)`. Робот — `Stack` + `Positioned(right: 8, bottom: 8)`, тугмалар қаторига `padding.only(right: 72)`.
- **Ҳафта қатори:** `Row` ичида 7 × `Expanded(flex: 1)` — ҳар устун қаторнинг 1/7 қисми (320 / 7 = 45,7 pt), қатъий кенглик берилмасин: карточка ўлчами ўзгарса ҳам тақсимот сақланади, оралиқ керак эмас; боғловчилар алоҳида `CustomPaint` қатлами доиралар **остида** (`Stack`: паст — чизиқлар, юқори — доиралар), чизиқ `y = 13`, кенглиги — устун кенглигича (олдинги доира марказидан шу доира марказигача), қалинлиги 2, ранг `brand`. Доира `Container(28, shape: circle, border: 2 шаффоф/brand/blue)`; расм `Image.asset` 18/16.
- **Токенлар:** `ThemeExtension<JuniorTokens>` — `card`, `row`, `line`, `text`, `text2`, `text3`, `slate`, `brand`, `brandSoft`, `blue`, `blueSoft`, `green2`, `greenSoft`, `red`, `pinkSoft`; радиуслар `rCard 18`, `rInner 12`, `rBtn 12`; `TextStyle` лар **янги шкала** бўйича: `title 17/600 h21`, `body 14/500 h18`, `small 13/500 h16`, `tiny 12/500 h15`, `btn 14/600 h17`, `num 22/600 h26`; оила `SFpro`. `.itxt` даги курсив «i» (600 14/1) ва `.d__c` ичидаги расм ўлчамлари қатъий — ўзгармайди.
