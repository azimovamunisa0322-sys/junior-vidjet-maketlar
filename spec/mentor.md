...
Карточка `.wcard`: 297×171 pt, падинг 12 → ички қути 273×147, радиус `--r-card` 18, фон `--c-card` `#FFFFFF`, соя `--shadow-card`. Тик оқим: сарлавҳа қатори → 8 → тана → 8 → пастки қатор (пастга ёпишган).
...
| Изоҳ блоки | `.wcard__note` | тўлиқ кенглик 273, падинг 8 10, радиус 12, ≤ 2 қатор (`line-clamp`), баландлик 31 / 46 | 500 12/15 | матн `--c-text`; фон одатий `--c-row` `#F8F8FC`; `--blue` `#E8F6FE`; `--green` `#ECFFDE`; `--pink` `#FFE4EA` |
| Асосий тугма | `.wcard__btn` | баландлик 36, радиус 12, падинг 0 14, кенглик 273 (робот бўлса 201) | 600 13/16 | фон `--c-brand` `#FF4F28`, матн `#FFFFFF` |
...
Баландлик бюджети (ҳар ҳолатда): сарлавҳа 18 + кичик сатр 17 = 35 → 8 → изоҳ ≤ 46 → 8 → тугма 36 = **133 ≤ 147**. Тана 60 pt жой олади, изоҳ 46 дан ошмайди (2 қаторли клэмп).

Робот геометрияси: `robot2` 64 pt қути y = 99…163, кўринадиган пикселлар y ≈ 104 дан (расм 512×427, вертикал марказланган); `robot5`/`robot7` 60 pt қути y = 103…163. Изоҳ y = 55…101 да тугайди → робот изоҳга тегмайди; тугма 201 pt кенгликда роботдан чапда.
...
| `loading` | Фақат биринчи юклашда, кэшда ҳолат йўқ бўлса (presence + охирги хабар сўрови кетяпти) | Скелет: сарлавҳа 118×16, кичик сатр 88×12, чип 72×22 пилл, изоҳ 273×46, тугма 273×36. Робот йўқ | — | — |
...
Матн узунлиги текшируви: сарлавҳа 14 белги (≤ 22); кичик сатрлар 12–23 UZ / 12–26 RU (≤ 30); чиплар 5–12 белги; тугмалар UZ 11–14, RU 8–18 белги («У меня есть вопрос» = 18, чегарада — `locked` да иконкасиз қолдирилди, 246 pt га сиғади); изоҳлар 39–61 белги, савол/жавоб клэмп билан кесилади. Сарлавҳа қатори энг кенг ҳолати `pending`: UZ ≈ 252/273 pt, RU («Отправлено: сегодня, 14:20» + «В ожидании») ≈ 270/273 pt — чегарада; рус тилида кичик сатрни «Сегодня, 14:20» деб қисқартириш мумкин.
...
| Карточканинг ўзи (сарлавҳа, изоҳ, робот) | бутун 297×171 | Тугма билан бир хил (чатни очади) — веб'дагидек «ўлик» юза қолдирилмайди. `locked` да: toast «Chat to'lovdan keyin ochiladi» (RU «Чат откроется после оплаты»). `loading` да ҳаракат йўқ |
...
- **Тузилиш:** `Stack` → [`Container`(297×171, `BoxDecoration(borderRadius: 18, color: card, boxShadow)`, `Padding(12)`, `Column`(head `Row`, `SizedBox(8)`, body `Expanded`, `SizedBox(8)`, foot `Row`)), `Positioned(right: 8, bottom: 8, child: Image.asset(robot, width: art))`]. Пастки қатор робот бўлса `Padding(right: 72)`.
...
- **Рус тили:** «У меня есть вопрос» тугма матни 18 белги — 201 pt тугмага сиғади (≈ 154 pt), лекин катта шрифт масштабида «Задать вопрос» га тушириш кўзда тутилсин.
